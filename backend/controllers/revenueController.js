...
exports.getRevenue = async (request, reply) => {
  try {
    ...
    if (!revenue) {
      return reply
        .code(404)
        .send({ success: false, message: "Revenue not found" });
    }

    logger.info(
      `Revenue ${request.params.id} retrieved by user ${request.user.id}`,
      {
        action: "getRevenue",
        userId: request.user.id,
...
// Create revenue
// Create revenue - SIMPLE MANUAL POPULATION
// Create revenue
exports.createRevenue = async (request, reply) => {
  try {
    ...
    // Populate the created revenue for response - FIXED APPROACH
    ...
    // Debug: Log populated result
    console.log("Populated revenue:", populatedRevenue);
    ...
    console.error("Create revenue error details:", error);
...
exports.updateRevenue = async (request, reply) => {
  try {
    ...
    logger.info(
      `Revenue ${request.params.id} updated by user ${request.user.id}`,
      {
...
exports.deleteRevenue = async (request, reply) => {
  try {
    ...
    logger.info(
      `Revenue ${request.params.id} deleted by user ${request.user.id}`,
      {
