...
const { generateRevenueId, generateExpenseId } = require('./numberSeriesService');
    ...
    worksheet.eachRow({ includeEmpty: false }, (row, rowIndex) => {
      ...
      try {
        ...
        if (!date || !typeName || !amount) {
          errors.push(`Row ${rowNumber}: Missing required fields (date, type, or amount)`);
          return;
        }
        ...
        if (!typeId) {
          errors.push(`Row ${rowNumber}: Invalid transaction type '${typeName}'`);
          return;
        }
